...
约束是字段中数据必须满足的表达式。不满足此表达式的数据无法被输入数据库中；字段的约束也无法设置为其中数据不满足的表达式。约束为空的字段可以接受任何数据。

格式是数据显示时计算的表达式，计算的结果会替代数据原值被显示。格式为空时，数据原值会被显示。

唯一性是一个布尔值（值为真或假），唯一性为真的字段不接受重复的数据。字段中存在重复数据时也不能将字段唯一性修改为真。
...
（尖括号中的项目为必选，方括号为可选，斜体为占位符，后同。）

例如：
```
add-field name "分数" constr "value>=0&&value<=100" format "value+\"分\"" info "单场比赛选手的得分"
```
这条指令将创建一个名为Score的字段。该字段限制数据必须是一个在0-100之间的数字，且显示时会给数字加上”分“字。没有```unique```标签，说明字段中可以包含重复值。